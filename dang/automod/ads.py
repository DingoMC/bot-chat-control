...
async def main (c_guild : discord.Guild, client, message):
    ...
    for i in range (0, len(s), 1):
        if str(s[i]) in str(message.content).lower():
            ...
            reason = 'Advertising - Containing server IP / name (' + str(cnt) + ')'
            warned_on = datetime.now()
            duration_mins = 60 * (2 ** (cnt - 1))
            ...
            print(ViolationMsg(user, str(message.content), 'AAD'))
            channel = client.get_channel(int(GetChannel('warnings')))
            await channel.send(content=None, embed=MemberWarned(user, reason, warned_on.strftime("%d/%m/%Y, %H:%M:%S"), expires_at.strftime("%d/%m/%Y, %H:%M:%S"), warn_id, 'dang$$ # AAD'))
            if countActiveWarnings(warn_list, user_id) >= 3:
                await AutoMute(c_guild, client, user, user_id)
            await message.delete()
            msg = 'Do not advertise {0.author.mention}!'
